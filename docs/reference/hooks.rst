...
~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Return a list of permission form field choices to be shown in the Groups administration area.

  Choices should be a standard Django Form/Model choices list of tuples.

  This should be used to limit/filter the available permission choices for a particular model or in a particular
  scenario. New custom permissions for a single model should be configured in `MANUAL_PERMISSIONS` in `settings.py`.
  Likewise, custom permissions to be applied to all models (and optionally filtered down by using a
  `register_permissions` hook) should be configured in `AUTOGENERATED_PERMSSIONS`. See `settings.py` for more
  information.
